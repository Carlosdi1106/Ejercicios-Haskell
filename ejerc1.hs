...
sumcuad a b c = if max a b == a then (a ^ 2 + (max b c) ^ 2) else (b ^ 2 + (max a c) ^ 2)
...
divModu (dividendo, divisor) = (dividendo `div` divisor, dividendo `mod` divisor)
